...
function getKeys(node, options) {
  var _options$dynamicMarke = options.dynamicMarker,
      dynamicMarker = _options$dynamicMarke === void 0 ? '*' : _options$dynamicMarke;
  ...
  if (node.type === 'BinaryExpression' && node.operator === '+') {
    var left = getKeys(node.left, options);
    var right = getKeys(node.right, options);
  ...
  if (node.type === 'ConditionalExpression') {
    return [].concat(_toConsumableArray(getKeys(node.consequent, options)), _toConsumableArray(getKeys(node.alternate, options)));
  }

  if (node.type === 'LogicalExpression') {
    switch (node.operator) {
      case '&&':
        return _toConsumableArray(getKeys(node.right, options));

      case '||':
        return [].concat(_toConsumableArray(getKeys(node.left, options)), _toConsumableArray(getKeys(node.right, options)));
...
function extractFromCode(code) {
  ...
  var _options$marker = options.marker,
      ...
      _options$dynamicMarke2 = options.dynamicMarker,
      dynamicMarker = _options$dynamicMarke2 === void 0 ? '*' : _options$dynamicMarke2;
